...
/** Named injection for the backend's base URL (provided by each app target). */
const val BACKEND_BASE_URL_QUALIFIER = "backendBaseUrl"

/** Named injection for the application-wide CoroutineScope (e.g. used by DB callbacks). */
const val APPLICATION_SCOPE_QUALIFIER = "applicationScope"

/**
 * Bindings that work on every Kotlin/JVM target this app supports today
 * (Android + Desktop). Platform modules add the AppDatabase, the
 * BackendTokenStore / SyncMetadataStore, and the BACKEND_BASE_URL_QUALIFIER
 * string.
 */
val coreSharedModule: Module = module {
    ...
    // DAO accessors - delegated to the platform-provided AppDatabase.
    ...
    // Database callback - the lambda breaks the AppDatabase <-> PlaylistDao cycle.
